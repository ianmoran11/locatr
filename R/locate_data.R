...
locate_data <-
  function(sheet = NULL, ...) {
    ...
    # Give expressions names, and convert to quosures where expressions are symbols
    # (fmt_ functions) or strings (spreadsheet ranges)

    openenv <- environment()

    if (length(filter_expresions_string) > 0) {
      filter_expresions_string <- filter_expresions_string %>%
        string_expressions_to_quosures(environ = openenv)

      filter_quosures_string_names <- filter_expresions_string %>%
        name_string_expressions(prefix = "flt_")

      names(filter_expresions_string) <- filter_quosures_string_names
    }

    if (length(filter_expresions_symbol) > 0) {
      filter_expresions_symbol <- filter_expresions_symbol %>%
        symbol_expressions_to_quosures(environ = openenv)

      filter_quosures_symbol_names <- filter_expresions_symbol %>%
        name_symbol_expressions(prefix = "flt_")
    ...
    if (length(filter_expresions_langage) > 0) {
      ...
      filter_quosures_language_names <- filter_expresions_langage %>%
        name_language_expressions(prefix = "flt_")
    ...
    data_cell_filter <-
      sheet %>%
      dplyr::mutate(!!!filter_quosures) %>% # Create flt_ vars
      dplyr::select(names(filter_quosures)) %>% # filter for flt_ vars
      as.list() %>%
      purrr::map(as.logical) %>% # Convert flt_ vars to logical
