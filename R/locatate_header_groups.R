...
#' The resulting data frame will contain the following columns: `.value`, `.direction`,
#' and `.header_label`.
#' These columns indicate the value, direction and name of the header columns that will be
#' produced in the tidy data frame produced by `migrate`.
#'
#' @param sheet data frame created by xlsx_cells
#' @param direction  a string indicating which type of headers are to be labelled. Options include
#'  compass direction or up/down/left/right.
#' @param .groupings expressions representing how header cells are differentiated. Most naturally
...
#' @param default_row_header_direction Indicates which direction is given to row headers by default.
...
#' @param .hook_if expression determining whether directions are hooked. That is, whether a header
...
#' @param .hook_if_rev expression determining whether directions are hooked.
...
locate_groups <-
  function(sheet = NULL, direction = "N", .groupings = groupings(fmt_alignment_indent),
    ...
    locate_groups_if(
      sheet = sheet_temp, direction = direction_temp, .groupings = .groupings_temp,
...
#' Locate multiple header groups
#' @description
#' This function conditionally adds direction annations to tidyxl data frame, specifying
#' annotions for several headers at once.
#' The resulting data frame will contain the following columns: `.value`, `.direction`,
#'  and `.header_label`.
#' These columns indicate the value, direction and name of the header columns that will be
#' produced in the tidy data frame produced by `migrate`.
...
#' @param sheet data frame created by xlsx_cells
#' @param direction  a string indicating which type of headers are to be labelled. Options
#' include compass direction or up/down/left/right.
...
#'  For example, see below that any(fmt_alignment_indent == 0)
#'  is used rather that fmt_alignment_indent == 0.
#' @param .hook_if_rev expression determining whether directions are hooked. That is, whether a
#'  header cell to the left of data cells should be
#'  annotated as "W" or "WNW". This argument contains an expression within the function `hook`.
#'  This expression must evaluate to a single value for each header group.
...
locate_groups_if <-
  function(sheet = NULL, ..., direction = "N", .groupings = groupings(fmt_alignment_indent),
    ...
    if (is.null(attr(sheet, "data_cells"))) {
      ...
      warning(paste0(
        "Data cells have yet not been located in this this dataframe with `locate_data()`.",
    ...
    header_fill_choice <- match.arg(arg = header_fill,
                                    choices = c("local_format_id", "style", "borders", "none"))
    groupings_temp <- .groupings
    default_col_header_direction_temp <- default_col_header_direction
    .hook_if_temp <- .hook_if
    .hook_if_rev_temp <- .hook_if_rev
    ...
    sheet <-
      ...
      dplyr::select(-.value.n, -.value.o, -.direction.n, -.direction.o, -.header_label.n,
                    -.header_label.o)
...
#' Groupings
#' @description
#' This functions passes grouping expressions to the .groupings argument of locate_groups.
#' It works most naturally with fmt_* functions.
